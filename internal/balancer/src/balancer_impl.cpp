...
void BalancerImpl::MergeSlices()
{
    ...
    std::unordered_map<uint64_t, std::list<TRange>::iterator> prevRange;
    for (size_t i = 0; i + 1 < ranges.size(); i++) {
        ...
        prevRange[secondRange->Start] = firstRange;
    }
    ...
    while (CurrentMappingStartIdToValue_.size() > minRangeCount) {
        ...
        auto oldLoad = CurrentMappingStartIdToValue_[firstRange.Start];
        auto oldSecondLoad = CurrentMappingStartIdToValue_[secondRange.Start];
        ...
        //mergeCostsToRange.erase(mergeCostsToRange.begin());
        if (prevRange.find(firstRange.Start) != prevRange.end()) {
            auto prev = prevRange[firstRange.Start];
            auto cost2 = CurrentMappingStartIdToValue_[prev->Start] + oldLoad;
            auto newCost = CurrentMappingStartIdToValue_[prev->Start] +  CurrentMappingStartIdToValue_[firstRange.Start];
            mergeCostsToRange.erase({cost2, prev->Start});
            mergeCostsToRange[{newCost, prev->Start}] = {prev, newFirstIt};
            nextRange[prev->Start] = newFirstIt;
        }
        prevRange.erase(secondRange.Start);

        if (nextRange.find(secondRange.Start) != nextRange.end()) {
            prevRange[nextRange[secondRange.Start]->Start] = newFirstIt;
            ...
            mergeCostsToRange.erase({oldSecondLoad + CurrentMappingStartIdToValue_[nextRange[firstRange.Start]->Start], secondRange.Start});
            mergeCostsToRange[{newCost, firstRange.Start}] = {newFirstIt, nextRange[firstRange.Start]};
        } else {
            nextRange.erase(firstRange.Start);
        }
    }
